...
def api_public_save():
    ...
    try:
        ...
        # 2. ENVIAR CORREOS AUTOMÁTICOS (Si falla el correo, no bloqueamos el éxito del registro)
        try:
...
# ==========================================
#  FUNCIÓN DE ENVÍO DE CORREOS (ESTILO PRO)
# ==========================================
def send_notification_emails(data):
    ...
    # --- 1. PREPARAR CORREO PARA LOGÍSTICA ---
    msg_logistics = MIMEMultipart()
    msg_logistics['Subject'] = f"🚚 NUEVA RECOLECCIÓN - {data.get('cliente')}"
    msg_logistics['From'] = f"Sievert Sistema <{sender_email}>"
    msg_logistics['To'] = logistics_email

    # HTML Logística (Estilo Imagen Adjunta)
    html_logistics = f"""
    <html>
        ...
        <div style="max-width: 600px; margin: 20px auto; background: white; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
            ...
            <div style="padding: 30px;">
                ...
                <div style="background-color: #f8fafc; border-left: 4px solid #52277c; padding: 15px; border-radius: 4px; margin-bottom: 20px;">
                    ...
                    <p style="margin: 5px 0;"><strong>📍 Dirección:</strong> {data.get('direccion')}</p>
                    <p style="margin: 5px 0;"><strong>🏙️ Ciudad:</strong> {data.get('ciudad')}</p>
                </div>
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <ul style="list-style: none; padding: 0; color: #333;">
                        ...
                        <li style="margin-bottom: 8px;">• <strong>Teléfono:</strong> {data.get('telefono')}</li>
    ...
    """
    msg_logistics.attach(MIMEText(html_logistics, 'html'))

    # --- 2. PREPARAR CORREO PARA CLIENTE ---
    msg_client = MIMEMultipart()
    msg_client['Subject'] = f"✅ Solicitud Recibida - Sievert LCD"
    msg_client['From'] = f"Sievert LCD <{sender_email}>"
    msg_client['To'] = data.get('email')
    ...
    with smtplib.SMTP(smtp_server, int(smtp_port)) as server:
        ...
        # Enviar a Logística
        server.sendmail(sender_email, logistics_email, msg_logistics.as_string())
        # Enviar a Cliente (Si puso correo)
        if data.get('email'):
            server.sendmail(sender_email, data.get('email'), msg_client.as_string())